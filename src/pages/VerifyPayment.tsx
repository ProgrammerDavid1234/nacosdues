...
const VerifyPayment: React.FC = () => {
  ...
  const fetchReceiptData = async (ref: string) => {
    try {
      ...
      // Save to nacos_payments for receipt page access
  ...
  const handleViewReceipt = () => {
    if (receiptData?.id) {
      // Navigate to receipt page with the payment ID
      navigate(`/receipt/${receiptData.id}`);
    } else if (paymentData?.id) {
      navigate(`/receipt/${paymentData.id}`);
    } else if (reference) {
      // Fallback to reference if no ID available
  ...
  const getStatusIcon = () => {
    if (verifying) {
      return <Loader2 className="h-24 w-24 animate-spin text-blue-600" />;
    }
      ...
      case 'success':
        return (
          <div className="relative">
            ...
            <CheckCircle2 className="h-24 w-24 text-green-600 relative z-10" strokeWidth={2} />
          </div>
        );
      case 'pending':
        return (
          <div className="relative">
            ...
            <AlertCircle className="h-24 w-24 text-yellow-600 relative z-10" strokeWidth={2} />
          </div>
        );
      case 'failed':
        return (
          <div className="relative">
            ...
            <XCircle className="h-24 w-24 text-red-600 relative z-10" strokeWidth={2} />
          </div>
        );
      default:
        return <AlertCircle className="h-24 w-24 text-gray-400" />;
    }
  };
  ...
  return (
    <Layout>
      <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-gray-50 via-white to-gray-100 py-12 px-4">
        <div className="container max-w-3xl mx-auto">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className={`bg-gradient-to-br ${colors.cardBg} rounded-3xl border-2 ${colors.border} shadow-2xl overflow-hidden`}
          >
            {/* Header */}
            ...
            <div className="p-8 md:p-12">
              ...
              >
                ...
                {fetchingReceipt && (
                  <p className="text-blue-600 text-sm bg-blue-50 border border-blue-200 rounded-lg p-3 mt-3 flex items-center justify-center gap-2">
              ...
              {(paymentData || receiptData) && (
                <motion.div
                  ...
                  className="bg-white rounded-2xl border-2 border-gray-200 p-6 mb-8 shadow-lg"
                >
              ...
              >
                ...
                {verificationStatus === 'failed' && (
                  <Button
                    onClick={handleRetry}
                    variant="outline"
                    className="w-full h-12 text-base font-semibold border-2 hover:bg-blue-50 hover:border-blue-600 hover:text-blue-600"
                    disabled={verifying}
                  >
                    {verifying ? (
                      <>
                ...
                {verificationStatus === 'pending' && (
                  <Button
                    onClick={handleRetry}
                    variant="outline"
                    className="w-full h-12 text-base font-semibold border-2 hover:bg-yellow-50 hover:border-yellow-600 hover:text-yellow-600"
                    disabled={verifying}
                  >
                    {verifying ? (
                      <>
              ...
              {verificationStatus === 'failed' && (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  transition={{ delay: 1 }}
                  className="mt-6 text-center text-sm text-gray-600 bg-gray-50 rounded-lg p-4 border border-gray-200"
                >
                  <p className="mb-2">
                    <strong>Need help?</strong>
                  </p>
                  <p>
                    If you continue to experience issues, please contact support with your payment reference: <span className="font-mono font-semibold">{reference}</span>
                  </p>
                </motion.div>
              )}
            </div>
